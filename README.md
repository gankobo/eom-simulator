...
**計算の核(core)と描画(viz/app)を分離**し、受け入れ基準(§11)を
単体テスト化してから 3D を構築している。

---

> ## ⚠️ 重要な但し書き
>
> これは外眼筋の運動学（キネマティクス）を可視化する **教育・研究用のデモ**です。
> 解剖パラメータは受け入れ基準（§11）を満たすよう構成した暫定値を含みます。
> **臨床判断・診療・手術計画・患者個別の予測には使用しないでください。医療機器ではありません。**
> 無保証で提供します。
...
- **手元で動かす:**
  ...
  ブラウザで開き、眼位を変えると6筋それぞれの作用（回旋/上下転/水平）が
  数値と3Dで更新されます。`listing.html` では Listing 則 / half-angle 則を可視化します。
...
P2 平衡ソルバ → P3 active pulley → P4 両眼 →
P5 臨床（麻痺/手術/Hess/Bielschowsky）。

## ライセンス

MIT License（[LICENSE](LICENSE) を参照）。© 2026 gankobo
